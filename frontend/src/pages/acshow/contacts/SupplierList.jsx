// frontend/src/pages/acshow/contacts/SupplierList.jsx
...
const SupplierList = () => {
  const { contacts, setContacts, loading, refresh } = useContacts('supplier');
  ...
  const handleDelete = async (target) => {
    ...
    // Keep backup in case the API call fails
    const previousContacts = contacts;

    // Remove from local state instantly (immediate UI feedback)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-gray-800">Suppliers <span className="text-gray-400 text-sm font-normal">(সরবরাহকারী)</span></h2>
      ...
      <ContactTable contacts={contacts} type="supplier" onEdit={handleEdit} onDelete={handleDelete} />

      {showForm && (
        <ContactFormModal contact={editingContact} type="supplier" onClose={() => { setShowForm(false); setEditingContact(null); }} onSuccess={handleSuccess} />
      )}
    </div>
  );
};
