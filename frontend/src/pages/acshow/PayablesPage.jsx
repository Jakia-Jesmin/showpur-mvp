// frontend/src/pages/acshow/PayablesPage.jsx
...
const PayablesPage = () => {
  ...
  const fetchPayables = useCallback(async () => {
    ...
    try {
      ...
      const urgent = data.filter(item => item.status === 'overdue');
      const urgentAmt = urgent.reduce((sum, item) => sum + parseFloat(item.remaining_amount || item.amount), 0);

      setTotalToPay(total);
      setUrgentCount(urgent.length);
      setUrgentAmount(urgentAmt);
    } catch (err) {
  ...
  const grouped = groupByParty(payables);
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="bg-white shadow-sm sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 py-3">
          ...
          <h1 className="text-lg font-bold text-gray-800 flex items-center gap-2">
            To Pay <span className="text-gray-400 text-sm font-normal">(দেনা)</span>
          </h1>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-4 space-y-4">
        ...
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-5 text-white shadow-lg">
          ...
          <h2 className="text-3xl font-black mb-4">৳{totalToPay.toLocaleString('en-IN')}</h2>
          <div className="flex flex-wrap gap-2 text-xs">
            <span className="bg-white/20 px-3 py-1 rounded-full font-medium">
              {grouped.length} Supplier{grouped.length !== 1 ? 's' : ''}
            </span>
            {urgentCount > 0 && (
              <span className="bg-red-500/40 px-3 py-1 rounded-full font-medium flex items-center gap-1 animate-pulse">
                <AlertTriangle size={12} /> {urgentCount} Urgent — ৳{urgentAmount.toLocaleString('en-IN')}
              </span>
            )}
          </div>
        </div>
          ...
          <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-4 flex items-start gap-3">
            ...
            <div>
              <p className="font-bold text-red-800 text-sm">
                {urgentCount} Overdue Payment{urgentCount > 1 ? 's' : ''} (সময় পার)
              </p>
              <p className="text-xs text-red-600 mt-0.5">
                Prioritize these to maintain supplier relationships. Total overdue: ৳{urgentAmount.toLocaleString('en-IN')}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-2">
          {[
            ...
            { key: 'pending', label: '⏳ Upcoming (আসন্ন)' },
            { key: 'overdue', label: '🚨 Overdue (সময় পার)', warning: true },
          ].map((tab) => (
            <button
              key={tab.key}
        ...
        <div className="space-y-3">
          {loading ? (
            ...
            <EmptyState
                icon="🎉"
                title="All Paid Up! (সব পরিশোধিত)"
                message="No pending payments. You're in good standing with all suppliers."
                variant="success"
              />
          )}
        </div>
...
const SupplierCard = ({ group, onTransactionClick }) => {
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-sm border overflow-hidden transition-all ${
      group.hasOverdue ? 'border-red-300 ring-1 ring-red-100' : 'border-gray-100'
    }`}>
      <button
      ...
      >
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold shrink-0 ${
            group.hasOverdue ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'
          }`}>
            {(group.party || '?')[0].toUpperCase()}
          </div>
          <div className="min-w-0">
            <p className="font-semibold text-gray-800 truncate">
              {group.party || 'Unknown Supplier'}
            </p>
            <div className="flex items-center gap-3 text-xs text-gray-500 mt-0.5">
              <span>{group.transactions.length} bill{group.transactions.length > 1 ? 's' : ''}</span>
        ...
        <div className="text-right mr-2">
          <p className={`font-bold ${group.hasOverdue ? 'text-red-600' : 'text-blue-700'}`}>
